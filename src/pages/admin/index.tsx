...
import { UserModel, GlobalModel } from '@/stores'
import { User } from '@/stores/admin/type'
...
const Admin = observer(() => {
  ...
  const toDelete = (item: User) => {
    ...
    if (
      item.is_admin &&
      model.users.filter(user => user.is_admin).length == 1
    ) {
  ...
  const columns = [
    ...
    },
    {
      title: '操作',
      dataIndex: 'action',
      render: (t: any, record: User) => (
        <span>
          <a onClick={() => toEdit(record)}>修改</a>
          <Divider type="vertical" />
          <a onClick={() => toDelete(record)} style={{ color: 'red' }}>
            删除
          </a>
        </span>
      ),
